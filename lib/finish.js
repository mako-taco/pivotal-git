...
module.exports = function (args) {
	check(function (err, pt) {
		...
		git.getBranch(function (err, branch) {
			...
			//go back to the original branch if we fail anywhere
			...
			git.config("branch." + branch + ".story", function (err, story_id) {
				...
				git.config("branch." + branch + ".parent", function (err, parent) {
					...
					git.pull(parent, function (err) {
						if(err) {
							console.error(("failed! please manually merge origin " + parent + " in to " + branch).red);
							process.exit(1);
						}
						...
						git.push(branch, function (err) {
							...
							git.checkout(parent, function (err) {
								...
								//get parent up to date
								console.log(("attempting to fast forward " + parent + " to origin " + parent).cyan);
								git.pull(parent, function (err) {
									if(err) {
										console.error("failed! please get " + parent + " up to date with origin");
										process.exit(1);
									}
										...
										git.merge(branch, "[#" + story.id + "] " + story.name, function (err) {
											...
											git.push(parent, function (err) {
												...
												pivotal.updateStory(pt.apikey, pt.project, story.id, update, function (err) {
													if(err) {
														console.error(new Error("Could not mark story as delivered: " + err.message))
													}
													...
													git.setConfigStore(store, function (err) {
														if(err) {
															console.error(new Error("Could not update story status in git config: " + err.message))
														}
